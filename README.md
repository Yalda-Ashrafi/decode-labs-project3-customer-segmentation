...
Elbow Method — plots the within-cluster sum of squares against K, and shows the point where adding more clusters stops meaningfully reducing error.

Silhouette Score — measures how tight each cluster is compared to how far apart the clusters are, confirming the segments are genuinely distinct rather than just numerous.
...
2D and 3D PCA projection with colour-coded segments.
...
scikit-learn — StandardScaler, PCA, KMeans and silhouette metrics
...
My role: sole developer. I designed and built the complete solution end to end, including the data preprocessing, the PCA implementation, K-Means clustering with elbow and silhouette validation, the cluster-to-persona translation logic, the modular code architecture, and the full Streamlit dashboard interface.
